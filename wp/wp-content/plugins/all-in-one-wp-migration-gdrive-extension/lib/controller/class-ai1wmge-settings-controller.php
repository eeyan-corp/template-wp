...
class Ai1wmge_Settings_Controller {
	...
	public static function index() {
		...
		Ai1wm_Template::render(
			...
			array(
				...
				'incremental'             => get_option( 'ai1wmge_gdrive_incremental', false ),
	...
	public static function settings( $params = array() ) {
		check_admin_referer( 'ai1wmge_gdrive_settings' );
		if ( ! current_user_can( 'export' ) ) {
			wp_die( __( 'You are not allowed to perform this action.', AI1WMGE_PLUGIN_NAME ) );
		}
		...
		if ( isset( $params['ai1wmge_gdrive_update'] ) ) {
			try {
				...
				// Set incremental
				if ( ! empty( $params['ai1wmge_gdrive_incremental'] ) ) {
				...
				$model->set_incremental_folder_id( trim( $params['ai1wmge_gdrive_incremental_folder_id'] ) );
				...
				if ( ( $user = wp_get_current_user() ) ) {
					$user->add_cap( 'ai1wm_gdrive_admin', $model->get_lock_mode() );
				}
	...
	public static function revoke( $params = array() ) {
		check_admin_referer( 'ai1wmge_gdrive_revoke' );
		if ( ! current_user_can( 'export' ) ) {
			wp_die( __( 'You are not allowed to perform this action.', AI1WMGE_PLUGIN_NAME ) );
		}
	...
	public static function selector( $params = array() ) {
		...
		try {
			...
			if ( $folder_id === 'drive' ) {
				$response = $gdrive->list_team_drives( $next_page_token, "title != 'incremental-backups' and mimeType = 'application/vnd.google-apps.folder'", array( 'orderBy' => 'folder,createdDate desc' ) );
			} else {
				$response = $gdrive->list_folder_by_id( $folder_id, $team_drive_id, $next_page_token, "title != 'incremental-backups' and mimeType = 'application/vnd.google-apps.folder'", array( 'orderBy' => 'folder,createdDate desc' ) );
			}
		} catch ( Ai1wmge_Error_Exception $e ) {
			status_header( 400 );
			echo json_encode( array( 'message' => $e->getMessage() ) );
	...
	public static function folder() {
		...
		echo json_encode(
			array(
				'id'         => $folder_id,
				'name'       => $folder_name,
				'app_folder' => get_option( 'ai1wmge_gdrive_app_folder', false ),
			)
		);
		exit;
	}
